...
<td className=""><div class="nr"><h2><b>{Nom} </b></h2><h3>- {region} -</h3>
...
De : <input type="date" required className="form-control bl"onChange={(event)=>{setde(event.target.value)}}/> <br></br>
jusqu'à :<input type="date" required className="form-control bl"onChange={(event)=>{seta(event.target.value)}} /><br></br>
<label >Nombre de personnes :</label>
<input type="Number" required class="form-control bl"onChange={(event)=>{setnombre(event.target.value)}} /><br></br>

<label for="exampleFormControlSelect1">Chambre :</label>
    <select class="form-control bl" value={Chambre} onChange={(event)=>{setChambre(event.target.value)}} required>
